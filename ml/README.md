...
1. **The business ham is synthetic.** 400 of the 3,120 ham examples are generated by
   ...
   there, and the held-out set still shows one failure — a pharmacy collection notice at
   73% — from a register it was never taught. A corpus of real modern mail is the honest
   fix; this is the closest substitute available offline. It is the same remedy
   `hard_negatives.py` applies to the phishing model, with the same caveat.
