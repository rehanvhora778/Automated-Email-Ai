...
**4,528 real emails** (2,720 ham / 1,808 spam) after de-duplication.

| Metric | Held-out test set (n = 906) |
|---|---|
| Accuracy | **0.9890** |
| Precision (spam) | **0.9783** |
| Recall (spam) | **0.9945** |
| F1 | **0.9863** |
| ROC-AUC | **0.9988** |
...
Of 906 test emails, **10 are wrong**: 8 false positives and 2 missed spam.
...
1. **`hard_ham` accuracy is 0.877.** The corpus includes a folder of genuine opt-in
...
2. **Corpus artifacts inflate the score.** `spamassassin` and `sightings` rank as strong
...
3. **The data is from 2002–2005.** Spam has changed considerably; this needs retraining
   on recent mail to stay honest.
4. **Text only.** No sender reputation, no SPF/DKIM headers, no user-interaction history —
...
# 4. spam:     parse -> train -> evaluate  (~40s)
